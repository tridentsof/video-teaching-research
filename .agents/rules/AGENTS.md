...
- **Knowledge Base (`kb/`)**: Before conducting research or making technical assumptions, check the `kb/` directory for existing domain knowledge, research documents, and project guidelines.
- **Memory (`memory/`)**: Inspect and update the `memory/` directory to maintain persistent project context, architectural decisions, and evolving workspace state across sessions.
...
- UI work / Frontend Design → `design-taste-frontend` & `frontend-ui-engineering` (must be applied together to produce high-end, responsive, accessible interfaces and avoid generic templated slop)
- UI Mockup Gate (Mandatory Pre-implementation) → Before implementing any Next.js page/component, the agent MUST generate 2–3 visual mockups (standalone HTML preview or PNG via `generate_image`) for user review and approval.
- Library / Framework Research → `context7` (must be used when researching software libraries/frameworks to fetch official documentation via API instead of guessing or making up signatures)
- Token Efficiency / Concise Output → `caveman` (must auto-apply to compress communication output and save tokens while preserving technical accuracy)
