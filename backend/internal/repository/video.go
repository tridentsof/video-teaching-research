...
func (r *VideoRepository) Create(ctx context.Context, v *model.Video) error {
	query := `
		INSERT INTO videos (id, teacher_id, title, blob_url, duration_sec, status, uploaded_at, user_id)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
	`
	_, err := r.db.Pool.Exec(ctx, query,
		v.ID, v.TeacherID, v.Title, v.BlobURL, v.DurationSec, v.Status, v.UploadedAt, v.UserID,
	)
	if err != nil {
...
func (r *VideoRepository) List(ctx context.Context) ([]model.Video, error) {
	query := `
		SELECT id, teacher_id, title, blob_url, duration_sec, status, uploaded_at, user_id
		FROM videos
